...
Set `ROBODOJO_TACTILE=1` and `ROBODOJO_TACTILE_CALIBRATION=<dir containing gelsight_r15_data/>` for any launch above. The simulator then records two observation-only tactile models on all four fingers, one frame per control step, into `sim/tactile/`. Contact reporting is enabled on the robot links; no collision, material or solver setting changes. `ROBODOJO_TACTILE=mounted` replaces GelSight with GelSight real, described below, and does change the grasp physics.

- **contact sensor**: an IsaacLab `ContactSensor` per finger link, filtered against every task block. It records per-block normal force (IsaacLab's force matrices are normal-only), PhysX friction, and the peak over the physics substeps. The video plots both magnitudes and their ratio against the effective friction coefficient (0.5 in these runs): a finger slides when the ratio reaches it.
- **GelSight**: a virtual 1 mm gel on each fingertip. Its indentation is ray-cast against the block colliders and rendered with IsaacLab's TacSL GelSight renderer. Only the image a real sensor gives is simulated; TacSL's penalty force field is not. The fingers are rigid, so the image shows contact shape and area; grip force comes from the contact model.
- **GelSight real**: a 2 mm compliant gel pad (PhysX compliant contact) on each flat fingertip, invisible to the RGB cameras, with a depth camera behind it inside the finger, as in TacSL's GelSight finger. Blocks sink into the gel, deeper the harder the grip; the camera's clipping range keeps only the gel layer, and the indentation it sees is rendered with the same R1.5 model.
